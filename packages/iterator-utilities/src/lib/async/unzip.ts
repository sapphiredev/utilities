 ...
 * @example
 * ```typescript
 * import { unzipAsync } from '@sapphire/iterator-utilities';
 *
 * const iterable = [[1, 'a'], [2, 'b'], [3, 'c']];
 * const [numbers, letters] = await unzipAsync(iterable);
 *
 * console.log(numbers);
 * // Output: [1, 2, 3]
 *
 ...
 */
export async function unzipAsync<const ElementType extends readonly any[]>(
	iterable: AsyncIterableResolvable<ElementType>
): Promise<UnzipAsyncIterable<ElementType>> {
	const resolvedIterable = toAsyncIterableIterator(iterable);
	const firstResult = await resolvedIterable.next();
	if (firstResult.done) {
	...
	for await (const entries of resolvedIterable) {
	...
	return results as UnzipAsyncIterable<ElementType>;
}

export type UnzipAsyncIterable<ElementType extends readonly any[]> = {
